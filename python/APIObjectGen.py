...
# for sms in APIObjectGen('sms', username, password, e):
#     print(sms)
#
...
class APIObjectGen:
    ...
    def get_objects(self, batchsize=100):
        p = {'limit':batchsize, 'start':self.start.strftime("%Y-%m-%dT%H:%M:%S.%f"), 'end':self.end.strftime("%Y-%m-%dT%H:%M:%S.%f")}
        
        result = requests.get("https://api.46elks.com/a1/{}".format(self.endpoint), params=p, auth=self.auth)
        if result.status_code != 200:
            raise Exception("API request failed with status {} and message: {}".format(result.status_code, result.text))
        
        data = json.loads(result.text)
        
        if 'next' in data:
            self.start = datetime.datetime.strptime(data['next'], '%Y-%m-%dT%H:%M:%S.%f')
        else:
            self.has_more = False
